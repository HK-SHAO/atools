...
- **零页面异常、零 `console.error`** 是它给出的确定性判据，也是这个文件里仅有的两项硬断言之一（另一项见下面 `AudioContext` 那条）。
- 主线程长任务只报数不设阈值（同 `perf.ts` 的理由）。当前基准：演示就绪 ~1.1 s、全程 **1 个**长任务
  （~130 ms，就是下面那条预热；它落在 worker 解码/编码期间，演示就绪的墙钟时间与不预热时相同）。
...
- 同一笔账的另一半：**设备上下文要提前建，别留到第一次点播放**。播放非它不可，所以
  `usePlayback` 在材料就绪（`duration > 0`）时就建好，「演示就绪」时计数必须 ≥1。
  判断方式是 `Page.addScriptToEvaluateOnNewDocument` 在文档脚本之前把 `AudioContext` 换成计数的子类
  （放在页面里注入就漏掉模块初始化那一段）。**可证伪**：换上改动前的产物 → 演示就绪时计数为 0，
  退 1 并报出这一条。
- **合成也不许留给点击。** 同一段注入里还包了一层 `window.Worker`，数「派出去的 `synthesise` 条数」
  与「还没回来的条数」。判据两句：演示就绪后 worker 必须已经**收过并算完**至少一次 `synthesise`
  （`posted ≥ 1 且 inflight = 0`），且点播放前后 `posted` 不许变。
  它不是时间阈值 —— 断言的是「点播放那一刻结果已在手上」这件结构事实。**两条都证伪过**：
  把预载整个拿掉 → 第一句红（`worker 没把「还原」算完`）；让预载算了却不写回 `job.audio` →
  第二句红（`点播放又让 worker 现算了一遍「还原」（1 → 2）`），两次都退 1。
  实测读数与 `main` 对照表见 `docs/algorithms.md`；当前基准：材料就绪后点播放到出声 **1 ~ 3 ms**。
