...
| `bun bench/ui.ts` | 是 | **界面链门禁**：真页面里把 演示 → 质检 → 重建相位 各走一遍，三次交互都要到位、页面零异常（含 `console.error`）；另报主线程长任务。它覆盖的是**主线程那份内核**的每一个消费者（读图 / 出图 / 质检读回）。先 `bun run build:web`。 |
...
现在这个状态不可表达，断言随之删除。搬一个模块进内核的固定动作因此只剩两步：
**把实现搬进 `moon/`，把它的行为测试搬进 `moon/*_wbtest.mbt`**；vitest 那侧只保留跨边界与浏览器行为
（样板：`moon/stub.mbt` + `moon/stub_wbtest.mbt`，`app/lib/stub.test.ts` 只剩取址与端到端）。
...
- 应用壳的期望清单不从构建脚本抄，而是从**运行中的 DOM** 取：入口脚本、样式、manifest、
  favicon、apple-touch-icon、preload 的数值内核，加上 `index.html` 本身与 manifest 自己引的那两份图标；
...
为什么要有它：数值实现整段搬进内核之后，「**哪一侧忘了挂内核**」在编译期与单元测试里都不现形 ——
读图链住在主线程（要 canvas），内核却是 worker 先挂上的那一份，而 wasm 实例不跨线程。
实测症状是：拖进一张频谱图后点「质检」，`.facts` 停在采样率那一行不动，控制台一条
「数值内核还没挂上」（报错点是 `stubFits`，离出错点很远）。

- 三次交互（演示 / 质检 / 重建相位）各自等自己的就绪态；「质检」那一行还要求 `原图 / 有损 / 半尺寸`
  三个标签齐全。
- **零页面异常、零 `console.error`** 是它给出的确定性判据，也是这个文件里唯一设阈值的两项。
- 主线程长任务只报数不设阈值（同 `perf.ts` 的理由）。当前基准：演示就绪 ~0.5 s、全程 **0 个**长任务。
- 它顺带守着一笔不属于内核的账：**解码上下文不许退回 `AudioContext`** —— 那个构造实测 135 ms。
...
- 动界面链、主线程那份内核、或解码上下文之后：`bun run build:web && bun bench/ui.ts`（顺带看一眼 `bun run perf` 的落 WAV 那半）。
