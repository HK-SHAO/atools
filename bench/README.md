...
| `bun run kernel` | 否 | **内核 A/B**：同一批用例在「内核挂上」与「摘掉内核（TS 参照实现）」两条路上各跑一遍，比时间也比结果。门禁是比值（同机同轮，机器噪声对消）。`SECS=30` 改素材时长。 |
...
数值内核（`moon/`）与它的 TS 参照实现并存，谁在跑由 `app/lib/dsp.ts` 的 `attachKernel` 决定 ——
本脚本就是那个开关：同一个用例跑两遍，一遍挂着内核、一遍摘掉。

- **判据是比值**：两条路在同一台机器、同一轮里跑，机器噪声对消，所以这条比值可以当门禁（先例是 `perf.ts` 的相位表倍数）。
- 两条路**交替**跑 3 轮、各自取最快一轮。最小值是无偏的估计量（调度与 GC 只会让某轮变慢）；
  必须交替是因为堆状态与频率调节会在两段之间漂移 —— 先跑完参照三轮再跑内核三轮时，**同一份代码**在两个位置能差 35%。
- 可逆档是**阴性对照**：它走 `synthesiseExact`，内核里没有对应实现，所以它的编码/还原两列应当恒为 1.00×（实测 1.00~1.01×）。
  这几列一旦偏离，说明测的不是内核，是噪声。
- 门禁三条：紧凑档总耗时 ≤ 0.95×（要**更快**，不是「没变慢」）；可逆档波形必须**逐位相同**（内核不许碰它）；
  全档 level 字节零差异、波形差 ≤ 峰值 5%。
- 开跑前先 `new Pair(512)` 断言 `via === "kernel"` —— 内核没挂上时 `Pair` 会**静默**退回参照实现，
  整轮 A/B 就会变成「自己跟自己比」而数字照样好看。
